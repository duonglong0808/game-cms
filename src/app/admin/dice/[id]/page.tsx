    ...
    <main className="min-h-full flex flex-col relative">
      ...
      {data?.length ? (
        <div className="main-page min-h-full flex-1">
          ...
          {diceDetailIdEdit && <PopupEditTransaction title="Cập nhật trạng thái ván chơi" textWarning="Chỉ cần ấn XÁC NHẬN không cần chọn trạng thái" id={+diceDetailIdEdit} data={dataDiceId || []} onCancel={() => dispatch(setDiceDetailIdEdit({ id: '' }))} onSubmit={handleUpdateStatusPaymentTransaction} />}
        </div>
      ) : (
        <></>
      )}
